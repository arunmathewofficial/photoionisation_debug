...
for frame in timeline:
    ...
    if found:
        ...
        IF_time.append((basic_data['sim_time'] * u.s).value / recombination_time)
    object.close()
...
factor = 3*QH/(4*math.pi*n_H*n_H*recombination_rate) # interms of parsec
Analytical_IF_position = []
for time in IF_time:
    Analytical_IF_position.append( pow(factor*(1.0 - math.exp(-time)), 1/3) / one_parsec)
...
plt.xlabel(r'$\rm time(t_{\rm rec})$')
plt.ylabel(r'$\rm Radius (R_S)$')
plt.text(6.5, 3.2, r'$\alpha_B = 2.5080\times10^{-13} cm^3/s $', horizontalalignment='center', verticalalignment='bottom', color='black')
plt.text(6.5, 3.5, r'$Q(H) = 4.255291\times10^{49}\, s^{-1}$', horizontalalignment='center', verticalalignment='bottom', color='black')
